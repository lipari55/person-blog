    ...
    <div id="blog-detail">
      ...
      <div id="blog-comments">
        ...
        >
          ...
          <div class="info">
            <div>
              <span style="margin-left:-84%;font-size:12px;color:#9b9b9b;"
                >{{ comment.authorName }}
              </span>
              <span style="float:right;font-size:12px;color:#9b9b9b;"
                >{{ comment.createTime }}
              </span>
            </div>
            <p style="text-align:left;margin:5px 0 10px">
              {{ comment.content }}
            </p>
          </div>
        </div>
...
export default {
    ...
    'methods': {
        deleteBlog () {
            this.$confirm('您确定要删除吗？', '提示', {
                ...
                .then(() => {
                    axios
                        .get('/api/delete_blog', { 'params': { 'id': this.id } })
                        .then(response => {
        ...
        initPageComments () {
            ...
            if (this.commentsLength > this.maxPageSize) {
                const start = (this.page - 1) * this.maxPageSize;
                const end = start + this.maxPageSize;
    ...
    created () {
        ...
        axios
            .get('/api/get_single_blog', { 'params': { 'id': that.id } })
            .then(response => {
                if (response.data[0].status === '200') {
        ...
        // 请求评论
        axios
            .get('/api/get_comments', { 'params': { 'id': that.id } })
            .then(response => {
                if (response.data[0].status === '200') {
                    this.blogComments = response.data.slice(1);
                    this.initPageComments();
