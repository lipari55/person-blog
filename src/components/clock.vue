<template>
  <div id="clockDiv">
    <canvas ref="clock" id="myclock" width="500px" height="500px"
...
export default {
    ...
    'methods': {
        ...
        drawDial (ctx) {
            ...
            for (let i = 1; i <= 12; i++) {
                ctx.save();
                const deg = -Math.PI / 2 + (i * Math.PI) / 6;
        ...
        drawAllHands (ctx) {
            ...
            const minuteAngle = (minute / 60) * 2 * Math.PI + secondAngle / 60;
            const hourAngle = (hour / 12) * 2 * Math.PI + minuteAngle / 12;
